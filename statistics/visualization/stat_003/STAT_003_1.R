...
#'
#' @examples
#' DATA_STAT_003_1(NULL)
#'
#' @export

DATA_STAT_003_1 <- function(json_content) {
  
  df <- data.frame(
         C1 = '<p style="font-family: URWDIN-Regular, sans-serif; font-weight: 600;
         font-size: 20px; line-height: 30px"><span style="color: #B43C23"><strong>MRP - Mieszkaniowy Rachunek Powierniczy </strong></span> jest rodzajem rachunku bankowego identyfikującym pojedynczą inwestycję chronioną przez Deweloperski Fundusz Gwarancyjny.</p>')
  
  return(df)
}

FLAT_DATA_STAT_003_1 <- function(json_content) {
...
INFOGRAPHIC_STAT_003_1 <- function(json_content) {

  tekst1 <- textGrob(label = "MRP - Mieszkaniowy Rachunek Powierniczy", gp = gpar(fontsize = 11, family = "URWDIN-Demi", col = "#B43C23"),hjust=0, x=0)
  tekst2 <- textGrob(label = "jest rodzajem rachunku bankowego identyfikującym pojedynczą inwestycję", gp = gpar(fontsize = 11, family = "URWDIN-Demi"), hjust = 0, x=-0.265)
  tekst3 <- textGrob(label = "chronioną przez Deweloperski Fundusz Gwarancyjny.", gp = gpar(fontsize = 11, family = "URWDIN-Demi"),hjust=0, x=0)
