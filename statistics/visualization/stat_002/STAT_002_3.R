...
DATA_STAT_002_3 <- function(json_content) {
#print(json_content)
  ...
  createDataFrame <- function(json_content) {
    if (length(json_content) == 0) {
      df_main <- data.frame(matrix(ncol=2,nrow=0, dimnames=list(NULL, c("NM4", "WOJEWODZTWO"))))
  ...
  df$NM4[is.na(df$NM4)] <- 0
  
  return(df)
}

INFOGRAPHIC_STAT_002_3 <- function(json_content) {
  df <- DATA_STAT_002_3(json_content)
  
  #df$NM4 <- format(as.numeric(df$NM4), big.mark=" ")
...
#    ggtitle("LICZBA PROWADZONYCH MRP W PODZIALE NA WOJEWÓDZTWA") +
    ...
    geom_text(data=df, aes(text2_x, text2_y, label=sapply(NM4, LABEL),
                            vjust = "inward", 
                            hjust =  ifelse(WOJEWODZTWO %in% c("pomorskie","warmińsko-mazurskie","podlaskie","kujawsko-pomorskie","mazowieckie","lubelskie","świętokrzyskie","podkarpackie")
    ...
    geom_segment(aes(x = long, y = lat,
                     xend =ifelse(str_count(NM4)>1 & WOJEWODZTWO %in% c("dolnośląskie", "lubuskie","łódzkie", "małopolskie","opolskie","śląskie","wielkopolskie","zachodniopomorskie")
                                  ,line_xend+str_count(NM4)*0.3-0.3,line_xend), yend = line_yend ), data = df, linetype="dashed", lwd=0.3) +
  ...
  legend_path <- paste(getwd(), '/icons/map_legend2.png', sep="")
