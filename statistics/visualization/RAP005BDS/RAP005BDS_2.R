#' Tworzy obiekt data.frame z podsumowaniem miar zamkniętych wątków zgrupowana w kategoriach
#'
#' Funkcja tworzy i zwraca obiekt data.frame z dziewięcioma kolumnami,
#' zawierającymi liczbę zamkniętych wątków i ich zgrupowaną reprezentację w kategoriach.
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP005BDS_prepared_data_3.
#' @return Obiekt data.frame z liczbą miar zamkniętych wątków.
#'
#' @examples
#' DATA_RAP005BDS_2(NULL)
#'
#' @importFrom utils assign
#' @importFrom getRAP005BDS_prepared_data_3
#' @export
DATA_RAP005BDS_2 <- function(json_content) {
  
  df_main <- getRap_005_3_prepared_data(json_content)
  df <- data.frame(
    C1 = c("Liczba zamkniętych wątków",
           "Liczba zamkniętych wątków kategorii Zobowiązania wobec DFG",
           "Liczba zamkniętych wątków kategorii Rozliczanie składek",
           "Liczba zamkniętych wątków kategorii Problemy techniczne",
           "Liczba zamkniętych wątków kategorii Niezgodność danych",
           "Liczba zamkniętych wątków kategorii Inne",
           "Liczba zamkniętych wątków kategorii Anomalie"),
...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP005BDS_prepared_data_3.
#' @return Obiekt ggplot z dwoma tabelami grob podsumowanie miar zamkniętych wątków.
#'
#' @examples
#' INFOGRAPHIC_RAP005BDS_2(NULL)
#'
#' @importFrom utils assign
...
#' @importFrom getRAP005BDS_prepared_data_3
#' @export
INFOGRAPHIC_RAP005BDS_2 <- function(json_content) {
  
  df_main <- getRap_005_3_prepared_data(json_content)
  
  
  
  df1 <- data.frame(var1=c(df_main$ZOBOWIAZANIA_DFG,"liczba zamkniętych
 wątków kategorii
  Zobowiązania wobec DFG"))
  df2 <- data.frame(var1=c(df_main$ROZLICZENIE_SKLADEK,"liczba zamkniętych
 wątków kategorii
  Rozliczanie składek"))
  df3 <- data.frame(var1=c(df_main$PROBLEMY_TECH,"liczba zamkniętych
 wątków kategorii
  Problemy techniczne"))
  

  df4 <- data.frame(var1=c(df_main$NIEZGODNOSC_DANYCH,"liczba zamkniętych
 wątków kategorii
  Niezgodność danych"))
  df5 <- data.frame(var1=c(df_main$INNE,"liczba zamkniętych
 wątków kategorii
 Inne"))
  df6 <- data.frame(var1=c(df_main$ANOMALIE,"liczba zamkniętych
 wątków kategorii
 Anomalie"))
  ...
  # title <- textGrob("MIARY ZAMKNIĘTYCH WĄTKÓW",gp=gpar(fontsize=16, fontface="bold", "URW DIN-DEMI"))
  # 
  ...
  grob_list <- list(table_grob_1,table_grob_2,table_grob_3,table_grob_4,
                    table_grob_5,table_grob_6,table_grob_7)
  ...
  grid_1 <- plot_grid(plot_1, plot_2, plot_3, ncol = 3, scale=1.1)
  grid_2 <- plot_grid(plot_4, plot_5, plot_6, ncol = 3, scale=1.1)
  
  
  plot <- plot_grid(plot_7,grid_1, grid_2, ncol=1)
