...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002DS_prepared_data_4.
#' @return Obiekt data.frame z liczbą uruchomień reguł walidacyjnych.
#'
#' @examples
#' DATA_RAP002DS_4(NULL)
#'
#' @importFrom utils assign
#' @importFrom getRAP002DS_prepared_data_4
#' @export

DATA_RAP002DS_4 <- function(json_content) {
  df_main_2 <- getRAP002DS_prepared_data_4(json_content)
...
#' @importFrom getRAP002DS_prepared_data_4
#' @export
INFOGRAPHIC_RAP002DS_4 <- function(json_content) {
  df_main_2 <-  getRAP002DS_prepared_data_4(json_content)
    ...
    ggplot(data = df[which(df$LICZBA_BLADOW>0),], fill = colors) +
      ...
      geom_point(
        aes(
          x = reorder(TYP_ZASILENIA, -Percent),
          y=0
          # y = min(ifelse(length(unique(TYP_ZASILENIA)) != 1, Percent, 0))
        ),
        size = 3,
        color = "#B43C23"
      ) +
