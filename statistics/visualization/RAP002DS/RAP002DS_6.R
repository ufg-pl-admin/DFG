#' Tworzy obiekt data.frame z liczbą błędnych uruchomień reguł walidacyjnych i liczbę korekt
#'
...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002DS_prepared_data_6.
#' @return Obiekt data.frame z liczbą błędnych uruchomień reguł walidacyjnych.
#'
#' @examples
#' DATA_RAP002DS_6(NULL)
#'
#' @importFrom utils assign
#' @importFrom getRAP002DS_prepared_data_6
#' @export
DATA_RAP002DS_6 <- function(json_content) {
  df_main_2 <- getRAP002DS_prepared_data_6(json_content)
  colnames(df_main_2) <- c("REGULA", "LICZBA_URUCHOMIEN", "LICZBA_KOREKT")
  
  return(df_main_2)
}

#' Tworzy obiekt data.frame z liczbą błędnych uruchomień reguł walidacyjnych
#'
#' Funkcja tworzy i zwraca obiekt data.frame z trzema kolumnami i wierszami,
...
INFOGRAPHIC_RAP002DS_6 <- function(json_content) {
  df_main_2 <- getRAP002DS_prepared_data_6(json_content)
  ...
  uruchomien_bladow <- melt(df_main_2, id.vars = 'NAZWA_REGULY')
  
  uruchomien_bladow <- uruchomien_bladow %>%
  ...
  plot <- if(length(json_content) == 0) { 
    ...
    ggplot(uruchomien_bladow, aes(x=0, y=0)) +
      xlab("") +
      ylab("") +
      theme_bw() +
    ...
    ggplot(uruchomien_bladow, aes(x = value, y = NAZWA_REGULY, fill = variable)) +
    ...
    theme(axis.text.y=element_text(face="bold", color = "black"), 
      ...
      scale_x_continuous(breaks = int_breaks, expand = c(0,0), labels = number_format(), limits=c(0,max(uruchomien_bladow$value)+0.1*max(uruchomien_bladow$value))) +
      scale_fill_manual(values=c("#B43C23", "#757575")) +
      theme(axis.text = element_text(family = "URW DIN-REGULAR")) +
      geom_text(data = uruchomien_bladow[uruchomien_bladow$variable == "Liczba korekt", ],
                aes(label = LABEL2(value)),
                color = "black",  family ="URWDIN-Demi", vjust = vjust_L_korekt, hjust = -0.5) +
      geom_text(data = uruchomien_bladow[uruchomien_bladow$variable == "Liczba uruchomień reguł walidacyjnych", ],
                aes(label = LABEL2(value)),
