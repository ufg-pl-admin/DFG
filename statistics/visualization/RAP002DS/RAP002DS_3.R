...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002DS_prepared_data_3.
#' @return Obiekt data.frame z liczbą uruchomień reguł walidacyjnych.
#'
#' @examples
#' DATA_RAP002DS_3(NULL)
#'
#' @importFrom utils assign
#' @importFrom getRAP002DS_prepared_data_3
#' @export

DATA_RAP002DS_3 <- function(json_content) {
  df_main_2 <- getRAP002DS_prepared_data_3(json_content)
...
#' @importFrom getRAP002DS_prepared_data_3
#' @export
INFOGRAPHIC_RAP002DS_3 <- function(json_content) {
  df_main_2 <-  getRAP002DS_prepared_data_3(json_content)
    ...
    ggplot(data = df[which(df$LICZBA_OSTRZEZEN>0),], fill = colors) +
      ...
      geom_segment(
        aes(
          ...
          y = 0,
          xend = reorder(TYP_ZASILENIA, -Percent),
          yend = ifelse(length(unique(TYP_ZASILENIA)) != 1, max(Percent), 1),
          group = colors
        ),
        linetype = "dashed",
        color = "gray12"
      ...
      geom_label(
        aes(
          x = reorder(TYP_ZASILENIA, -Percent),
          y = ifelse(length(unique(TYP_ZASILENIA)) != 1, max(Percent), 1),
          label = paste0(round((Percent * 100),2), "%")
        ),
        position = position_stack(vjust = 0.5),
        fill = alpha("white", 0),
