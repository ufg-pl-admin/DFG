...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002DS_prepared_data_5.
#' @return Obiekt data.frame z liczbą uruchomień reguł walidacyjnych.
#'
#' @examples
#' DATA_RAP002DS_5(NULL)
#'
#' @importFrom utils assign
#' @importFrom getRAP002DS_prepared_data_5
#' @export
DATA_RAP002DS_5 <- function(json_content) {
  df_main_2 <- getRAP002DS_prepared_data_5(json_content)
...
INFOGRAPHIC_RAP002DS_5 <- function(json_content) {
  df_main_2 <- getRAP002DS_prepared_data_5(json_content)
  ...
  plot <- if(length(json_content) == 0) { 
    ...
    ggplot(uruchomien_ostrzezen, aes(x = value, y = NAZWA_REGULY, fill = variable)) +
      geom_bar(stat = "identity", position = 'dodge', alpha = 1, width = 0.8) +
      xlab("") + 
      ylab("") +
      theme_bw() +
