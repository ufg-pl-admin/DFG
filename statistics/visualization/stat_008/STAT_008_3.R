...
#' "MRP" zawierającą tekstowe opisy grup MRP oraz "NM4" zawierającą liczbę inwestycji wg daty otwarcia
...
#'
#' @examples
#' DATA_STAT_008_3(json_content)
#'
#' @export
source(paste(getwd(), '/utils/int_breaksUtil.R', sep=""))
source(paste(getwd(), '/utils/labelUtil.R', sep=""))

DATA_STAT_008_3 <- function(json_content) {
  df_main <- getStat_008_prepared_data_3(json_content)
  df_main <- df_main[order(df_main$KAT),]
  return(df_main)
}

INFOGRAPHIC_STAT_008_3 <- function(json_content) {
  df_main_2 <- getStat_008_prepared_data_3(json_content)
  ...
  plot <- if (all(df_main_2$NM4 == 0)) { 
    
    ggplot(df_main_2, aes(x = NM4, y = MIESIAC)) +
      ...
      scale_x_continuous(breaks = int_breaks, expand = c(0,0), limits=c(0,max(df_main_2$NM4)+0.1*max(df_main_2$NM4))) +
      scale_y_discrete(limits = levels(df_main_2$NM4)) +
    ...
    scale_x_continuous(breaks = int_breaks, expand = c(0,0), limits=c(0,max(df_main_2$NM4)+0.1*max(df_main_2$NM4))) +
    scale_y_discrete(limits = levels(df_main_2$NM4))  +
    geom_text(aes(label = sapply(NM4, LABEL)),  color = "black",  family ="URWDIN-Demi", hjust =-0.5)
  }
