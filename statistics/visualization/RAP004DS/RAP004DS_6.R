...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP004DS_prepared_data_6.
#' @return Obiekt data.frame z miarami kompletności danych w podziale na statusy dla umów deweloperskich.
#'
#' @examples
#' DATA_RAP004DS_6(NULL)
#'
#' @importFrom utils assign
#' @importFrom getRAP004DS_prepared_data_6
#' @export
DATA_RAP004DS_6 <- function(json_content) {
...
#' @importFrom getRAP004DS_prepared_data_6
#' @export
INFOGRAPHIC_RAP004DS_6 <- function(json_content) {
  df_main_2 <-  getRAP004DS_prepared_data_4(json_content)
  ...
  if(length(json_content) == 0 || all(df_main_2 == 0)) {
    ...
    plt1 <- ggplot(data = df, fill = colors) +
      ...
      theme(
        ...
        axis.line = element_blank(),
        axis.text.y = element_blank(),
        axis.text.x = element_text(color = "black", family = "URW DIN-REGULAR", face = "plain", size = 15),
        axis.ticks = element_blank(),
        axis.title = element_blank(),
        plot.title = element_blank()
      ) +
