...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP004_prepared_data_6.
...
#' @importFrom getRAP004_prepared_data_6
#' @export
DATA_RAP004_6 <- function(json_content) {
  df_main_2 <- getRAP004_prepared_data_4(json_content)
...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP004_prepared_data_6.
#' @return Obiekt ggplot prezentujący kompletność danych dla umów deweloperskich.
#'
#' @examples
#' INFOGRAPHIC_RAP004_6(NULL)
#'
#' @importFrom utils assign
...
INFOGRAPHIC_RAP004_6 <- function(json_content) {
  df_main_2 <-  getRAP004_prepared_data_4(json_content)
  ...
  if(length(json_content) == 0 || all(df_main_2 == 0)) {
    ...
    #df <- cbind(df, colors=c('A', 'B', 'C', 'D', 'E'))
    ...
    #   ggtitle("STATUSY KOMPLETNOSCI DANYCH O UMOWACH DEWELOPERSKICH") +
    ...
    # sorted_percent <- sort(unique(df$Percent), decreasing = TRUE)
    # second_max_value <- sorted_percent[2]
