...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP004_prepared_data_8.
...
#' @importFrom getRAP004_prepared_data_8
#' @export
source(paste(getwd(), '/utils/int_breaksUtil.R', sep=""))
source(paste(getwd(), '/utils/labelUtil.R', sep=""))  

DATA_RAP004_8 <- function(json_content) {
  df_main_2 <- getRAP004_prepared_data_5(json_content)
...
#' @importFrom getRAP004_prepared_data_8
#' @export
INFOGRAPHIC_RAP004_8 <- function(json_content) {
  df_main_2 <-  getRAP004_prepared_data_5(json_content)
...
plot <- if(length(json_content) == 0 || all(df$LICZBA == 0)) { 
