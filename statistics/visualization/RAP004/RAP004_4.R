...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP004_prepared_data_4.
...
#' @importFrom getRAP004_prepared_data_4
#' @export
DATA_RAP004_4 <- function(json_content) {
  df_main_2 <- getRAP004_prepared_data_3(json_content)
...
INFOGRAPHIC_RAP004_4 <- function(json_content) {
  df_main_2 <-  getRAP004_prepared_data_3(json_content)
  ...
  # Zastąpienie wartości zerowych 1
    ...
    # new_max_y <-
    #   df %>%
    #   mutate(nchar = nchar(STATUS),
    #          new_max_y = Percent * (1 + 0.01 * nchar)) %>%
    #   pull(new_max_y) %>%
    #   max()
   ...
   # expand_limits(y = c(0, new_max_y))
    ...
    plot <- plt1 +
      scale_y_continuous(
        limits = c(-0.2, max_value + 0.09),#limits = c(-0.2, ifelse(percent == , max_value + 0.15, max_value + 0.09),
        #expand = c(0, 0),
        breaks = c((max_value*1/4), (max_value*2/4), (max_value*3/4), max_value)
      ) #+ expand_limits(y = c(0, new_max_y))
    #plot <- plot + theme(plot.margin = margin(100, 100, 100, 100))

    # text = paste("\n   Taki teskt sobie będzie\n",
    #              "       Ładny napis informacyjny, że\n",
    #              "       no niestety nie ma danych")
    # tekst <- ggplot() +
