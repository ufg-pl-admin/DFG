#' Tworzy obiekt data.frame z miar poprawnie przekazanych zasileń WPLMRP, INSKLA i DANEAN.
#'
#' Funkcja tworzy i zwraca obiekt data.frame z trzema kolumnami,
#' zawierającymi miary poprawnie przekazanych zasileń WPLMRP, INSKLA i DANEAN.
#'
#' @examples
#' DATA_RAP003_3(NULL)
#'
...
DATA_RAP003_3 <- function(json_content) {
...
#' zawierającymi miary poprawnie przekazanych zasileń WPLMRP, INSKLA i DANEAN.
#'
#' @return Obiekt ggplot z przekazanych zasileń WPLMRP, INSKLA i DANEAN.
#'
#' @examples
#' INFOGRAPHIC_RAP003_3(NULL)
#'
...
INFOGRAPHIC_RAP003_3<- function(json_content) {
  df <- DATA_RAP003_3(json_content)
  
  
  left_max_limit <- round(max(max(df$LICZBA_ZASILEN) + 0.4 * max(df$LICZBA_ZASILEN), 2))
  left_length_out = 5
  if (left_max_limit < 5) {
    left_length_out = left_max_limit
  } else {
    left_length_out = 5
  }
  ...
  plot1 <- ggplot(df, aes(x=LICZBA_ZASILEN, y=RODZAJ_ZASILENIA)) +
    ...
    scale_x_continuous(breaks = seq(0, max(df$LICZBA_ZASILEN), length.out = left_length_out),
                    expand = c(0,0), 
