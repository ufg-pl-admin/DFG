#' Tworzy obiekt data.frame z dziesięcioma miejscowościmi pod względem największej liczby inwestycji
#'
#' Funkcja tworzy i zwraca obiekt data.frame z dwoma kolumnami: "MIEJSCOWOSC" i "L_UMOW",
#' zawierającymi dziesięcięć miejscowości pod względem największej liczby inwestycji.
#'
#' @param json_content Argument wykorzystywany w funkcji getStat_010_prepared_data i getCorrectedWords.
#' @return Obiekt data.frame z dziesięcioma miejscowościmi pod względem największej liczby inwestycji.
#'
#' @examples
#' DATA_STAT_010_3(NULL)
#'
#' @importFrom utils assign
#' @importFrom getCorrectedWords
#' @importFrom getStat_010_prepared_data
#' @export
DATA_STAT_010_3 <- function(json_content) {
  df_main_2 <- getStat_010_prepared_data_3(json_content)
...
INFOGRAPHIC_STAT_010_3 <- function(json_content) {
  df_main <- getStat_010_prepared_data_3(json_content)
  df_main$L_UMOW <- as.numeric(df_main$L_UMOW)
  ...
  colnames(TOP_TEN) <- c("Lp.", "Miejscowość","Liczba umów deweloperskich", "Udział")
  ...
  # title <- textGrob("TOP 10 miejscowości wg liczby umów deweloperskich",gp=gpar(fontsize=15, fontface="bold", "URW DIN-DEMI"))
  # 
