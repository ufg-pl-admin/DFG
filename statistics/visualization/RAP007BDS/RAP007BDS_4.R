#' Tworzy obiekt data.frame z miarami spraw niezgodności dla umów MRP w podziale na statusy
#'
...
DATA_RAP007BDS_4 <- function(json_content) {
  df_main_2 <- getRAP007BDS_prepared_data_3(json_content)
...
#' @importFrom getRAP007BDS_prepared_data_4
#' @export
INFOGRAPHIC_RAP007BDS_4 <- function(json_content) {
  df_main_2 <-  getRAP007BDS_prepared_data_3(json_content)
  ...
  plot <- if (all(df$LICZBA == 0)) {
    ...
    ggplot(df, aes(x=STATUS, y=LICZBA, fill=STATUS)) +
     ...
     scale_fill_manual(values=c('#B43C23','#B43C23','#B43C23','#B43C23','#B43C23','#B43C23','#B43C23','#B43C23')) +
      ...
      theme(plot.title = element_blank()) +#element_text(hjust = 0.5, size = 12,margin=margin(0,0,18,0))) +
