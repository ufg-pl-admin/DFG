...
DATA_RAP007BDS_7 <- function(json_content) {
  df_main_2 <- getRAP007BDS_prepared_data_6(json_content)
...
#' @importFrom getRAP007BDS_prepared_data_6
#' @export
INFOGRAPHIC_RAP007BDS_7 <- function(json_content) {
  df_main_2 <-  getRAP007BDS_prepared_data_6(json_content)
  ...
  plot <- ggplot(df_main_2, aes(x=L_NIEZGODNOSCI, y=MIESIAC)) +
    geom_bar(stat='identity', position='dodge', alpha=1, width=.8, fill= "#B43C23") +
    xlab("") + 
    ylab("") +
    theme_bw() +
