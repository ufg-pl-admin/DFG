#' Tworzy obiekt data.frame z miarami spraw niezgodności dla umów deweloperskich w podziale na statusy
#'
...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP007BDS_prepared_data_5.
#' @return Obiekt data.frame z miarami spraw niezgodności w podziale na statusy.
#'
#' @examples
#' DATA_RAP007BDS_5(NULL)
#'
#' @importFrom utils assign
#' @importFrom getRAP007BDS_prepared_data_5
#' @export
source(paste(getwd(), '/utils/int_breaksUtil.R', sep=""))
source(paste(getwd(), '/utils/labelUtil.R', sep=""))  

DATA_RAP007BDS_5 <- function(json_content) {
  df_main_2 <- getRAP007BDS_prepared_data_4(json_content)
  colnames(df_main_2) <- c("L_WWER", "L_ROZW", "L_ODRZ", "L_DODOPR", "L_REKL", "L_ZAMK", "L_AUTZAMK")
  df <- data.frame(C1 = c("Liczba spraw niezgodności o statusie W weryfikacji", "Liczba spraw niezgodności o statusie Rozwiązana",
                          "Liczba spraw niezgodności o statusie Odrzucona", "Liczba spraw niezgodności o statusie Do doprecyzowania", "Liczba spraw niezgodności o statusie Reklamacja",
                          "Liczba spraw niezgodności o statusie Zamknięta", "Liczba spraw niezgodności o statusie Zamknięta automatycznie"),
                   C2 = c(df_main_2$L_WWER, df_main_2$L_ROZW, df_main_2$L_ODRZ, df_main_2$L_DODOPR, df_main_2$L_REKL, df_main_2$L_ZAMK, df_main_2$L_AUTZAMK))
...
#' zawierającymi miary spraw niezgodności w podziale na statusy.
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP007BDS_prepared_data_4.
#' @return Obiekt grob_table prezentujący miary spraw niezgodności w podziale na statusy.
#'
#' @examples
#' INFOGRAPHIC_RAP007BDS_4(NULL)
#'
#' @importFrom utils assign
#' @importFrom gridExtra tableGrob
#' @importFrom ggplot2 ggdraw draw_image theme_void annotation_custom plot_grid
#' @importFrom getRAP007BDS_prepared_data_4
#' @export
INFOGRAPHIC_RAP007BDS_5 <- function(json_content) {
  df_main_2 <-  getRAP007BDS_prepared_data_4(json_content)
