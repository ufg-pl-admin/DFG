...
#' "WOJEWODZTWO" zawierającą grupy standaryzowanych nazw województw oraz "L_INWESTYCJI" zawierającą liczbę inwestycji
...
#'
#' @examples
#' DATA_STAT_009_2(json_content)
#'
#' @export
DATA_STAT_009_2 <- function(json_content) {
  df_main <- getStat_009_prepared_data_2(json_content)
  df_main <- df_main %>% arrange(desc(L_INWESTYCJI))

  return(df_main)
}

INFOGRAPHIC_STAT_009_2 <- function(json_content) {
  df <- DATA_STAT_009_2(json_content)
  df <- df %>% arrange(desc(L_INWESTYCJI))
  ...
  plot <- ggplot(df, aes(x=L_INWESTYCJI, y=reorder(WOJEWODZTWO, L_INWESTYCJI))) +
    ...
    scale_x_continuous(breaks = int_breaks, expand = c(0, 0), labels = number_format(), limits = c(0, max(df$L_INWESTYCJI) + 0.1 * max(df$L_INWESTYCJI))) +
    scale_y_discrete(limits = levels(df$L_INWESTYCJI))  +
    geom_text(aes(label = sapply(L_INWESTYCJI, LABEL)), color = "black", family = "URWDIN-Demi", hjust = -0.5)
