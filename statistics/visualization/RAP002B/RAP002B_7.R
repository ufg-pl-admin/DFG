...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002B_prepared_data_7.
...
#' @importFrom getRAP002B_prepared_data_7
#' @export
DATA_RAP002B_7 <- function(json_content) {
  df_main_2 <- getRAP002B_prepared_data_7(json_content)
  colnames(df_main_2) <- c("L_SKORYGOWANYCH", "BANK")
  ...
  lines <- data.frame(
    ...
    PERCENT = c(round(TOP_5$L_SKORYGOWANYCH/sum(df_main$L_SKORYGOWANYCH) * 100))
  )
  other <- data.frame(LP = 6,
                      BANK ='pozostałe',
                      L_SKORYGOWANYCH = sum(df_main$L_SKORYGOWANYCH) - sum(TOP_5$L_SKORYGOWANYCH),
                      PERCENT = round(((sum(df_main$L_SKORYGOWANYCH) - sum(TOP_5$L_SKORYGOWANYCH))/sum(df_main$L_SKORYGOWANYCH)) * 100))
...
#' @importFrom getRAP002B_prepared_data_7
#' @export
INFOGRAPHIC_RAP002B_7 <- function(json_content) {
  df_main <- getRAP002B_prepared_data_7(json_content)
  df_main$L_SKORYGOWANYCH <- as.numeric(df_main$L_SKORYGOWANYCH)
  ...
  PIEDONUT_BANK <- if (length(json_content) == 0) {
