...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002_prepared_data_4.
...
#' @importFrom getRAP002_prepared_data_4
#' @export

DATA_RAP002_4 <- function(json_content) {
  df_main_2 <- getRAP002_prepared_data_4(json_content)
...
#' @importFrom getRAP002_prepared_data_4
#' @export
INFOGRAPHIC_RAP002_4 <- function(json_content) {
  df_main_2 <-  getRAP002_prepared_data_4(json_content)
  ...
  PIEDONUT_TYP_ZASILENIA <- if (length(unique(df$TYP_ZASILENIA)) == 1) {
    ...
    ggplot(data = df[which(df$LICZBA_BLADOW>0),], fill = colors) +
      ...
      geom_segment(
        aes(
          x = reorder(TYP_ZASILENIA, -Percent),
          # y = min(Percent),#ifelse(length(unique(TYP_ZASILENIA)) != 1, min(Percent), 0),#min(Percent)
          y = 0,
          xend = reorder(TYP_ZASILENIA, -Percent),
          yend = max(Percent), #ifelse(length(unique(TYP_ZASILENIA)) != 1, max(Percent), 1),#max(Percent)
          group = colors
        ),
        linetype = "dashed",
        color = "gray12"
      ...
      geom_label(
        aes(
          x = reorder(TYP_ZASILENIA, -Percent),
          y = max(Percent), #ifelse(length(unique(TYP_ZASILENIA)) != 1, max(Percent), 1),#max(Percent)
          label = paste0(round((Percent * 100),2), "%")
        ),
        position = position_stack(vjust = 0.5),
        fill = alpha("white", 0),
