...
#' zawierającymi liczbę wystąpień ostrzeżeń per typ zasilenia.
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002_prepared_data_3.
...
#' @importFrom getRAP002_prepared_data_3
#' @export

DATA_RAP002_3 <- function(json_content) {
  df_main_2 <- getRAP002_prepared_data_3(json_content)
  colnames(df_main_2) <- c("TYP_ZASILENIA", "LICZBA_OSTRZEZEN")
...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP002_prepared_data_3.
#' @return Obiekt ggplot z wykresem radarowym opisującym liczbę ostrzeżeń.
#'
#' @examples
#' INFOGRAPHIC_RAP002_3(NULL)
#'
#' @importFrom utils assign
...
INFOGRAPHIC_RAP002_3 <- function(json_content) {
  df_main_2 <-  getRAP002_prepared_data_3(json_content)
  
  colnames(df_main_2) <- c("TYP_ZASILENIA", "LICZBA_OSTRZEZEN")
  
  df <- df_main_2 %>%
    mutate(Percent=df_main_2$LICZBA_OSTRZEZEN/sum(df_main_2$LICZBA_OSTRZEZEN)) %>%
    arrange(desc(df_main_2$LICZBA_OSTRZEZEN))
  ...
  df <- bind_rows(
    df,
    df %>%
      filter(length(unique(TYP_ZASILENIA)) == 1) %>%
      mutate(
        Percent = 0,
      )
  )
  ...
  PIEDONUT_TYP_ZASILENIA <- if (length(unique(df$TYP_ZASILENIA)) == 1) {
    ...
    ggplot(data=df,aes(x=TYP_ZASILENIA,y=Percent,fill=colors))+
      ...
      geom_segment(
        aes(
          ...
          y = 0,
          # y = ifelse(length(unique(TYP_ZASILENIA)) != 1, min(Percent), 0),#min(Percent)
      ...
      geom_point(
        aes(
          x = reorder(TYP_ZASILENIA, -Percent),
          y = 0
          # y = min(ifelse(length(unique(TYP_ZASILENIA)) != 1, Percent, 0))#min(Percent)
        ),
        size = 3,
        color = "#B43C23"
      ) +
    ...
    ggplot(data=df, aes(x=0, y=0)) +
      ...
      ggplot(data = df[which(df$LICZBA_OSTRZEZEN>0),], fill = colors) +
    geom_hline(
      aes(yintercept = y),
