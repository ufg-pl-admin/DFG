...
#'
#' @param json_content Argument wykorzystywany w funkcji getRAP009B_prepared_data_2.
...
#' @importFrom getRAP009B_prepared_data_2
#' @export

DATA_RAP009B_2 <- function(json_content) {
  
 df_main <- getRap_009B_2_prepared_data(json_content)
...
#' @importFrom getRAP009B_prepared_data_2
#' @export
INFOGRAPHIC_RAP009B_2 <- function(json_content) {
  df_main <- DATA_RAP009B_2(json_content)
  
# Check the length of unique values in DATA_ANOMALII
  ...
  # plot <- ggplot(data = df_main, mapping = aes(x = DATA_ANOMALII)) +
  #   # geom_line(data = df_main %>% filter(ZAWMRP > 0),mapping = aes(y = ZAWMRP, group = 1, color="ZAWMRP"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(ZAUMDE > 0),mapping = aes(y = ZAUMDE, group = 1, color="ZAUMDE"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(ZMUMDE > 0),mapping = aes(y = ZMUMDE, group = 1, color="ZMUMDE"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(WPLMRP > 0),mapping = aes(y = WPLMRP, group = 1, color="WPLMRP"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(DANEAN > 0),mapping = aes(y = DANEAN, group = 1, color="DANEAN"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(INSKLA > 0),mapping = aes(y = INSKLA, group = 1, color="INSKLA"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(WYTMRP > 0),mapping = aes(y = WYTMRP, group = 1, color="WYTMRP"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(WSTRZD > 0),mapping = aes(y = WSTRZD, group = 1, color="WSTRZD"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(PRZEKS > 0),mapping = aes(y = PRZEKS, group = 1, color="PRZEKS"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(ZAMMRP > 0),mapping = aes(y = ZAMMRP, group = 1, color="ZAMMRP"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(ROUMDE > 0),mapping = aes(y = ROUMDE, group = 1, color="ROUMDE"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(UPAPOS > 0),mapping = aes(y = UPAPOS, group = 1, color="UPAPOS"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(WYPMRP > 0),mapping = aes(y = WYPMRP, group = 1, color="WYPMRP"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(ZRWNAB > 0),mapping = aes(y = ZRWNAB, group = 1, color="ZRWNAB"), lwd=0.9) +
  #   # geom_line(data = df_main %>% filter(UPADEW > 0),mapping = aes(y = UPADEW, group = 1, color="UPADEW"), lwd=0.9) +
  #   geom_line(data = df_main, mapping = aes(y = ZAWMRP, group = 1, color="ZAWMRP"), lwd=0.9) +
  #   geom_line(data = df_main, mapping = aes(y = ZAUMDE, group = 1, color="ZAUMDE"), lwd=0.9) +
  #   geom_line(data = df_main, mapping = aes(y = ZMUMDE, group = 1, color="ZMUMDE"), lwd=0.9) +
  #   geom_line(data = df_main, mapping = aes(y = WPLMRP, group = 1, color="WPLMRP"), lwd=0.9) +
  ...
  #   geom_line(data = df_main, mapping = aes(y = ZRWNAB, group = 1, color="ZRWNAB"), lwd=0.9) +
  #   geom_line(data = df_main, mapping = aes(y = UPADEW, group = 1, color="UPADEW"), lwd=0.9) +
  #   # geom_point(data = df_main[!duplicated(df_main$DATA_ANOMALII) | duplicated(df_main$DATA_ANOMALII, fromLast = TRUE), ],
  #   #            aes(y = ZAWMRP, color = "ZAWMRP"), size = 3) +  
  #   # Conditional geom_point layer
  #   if (length(unique(df_main$DATA_ANOMALII)) == 1) {
  #     geom_point(data = df_main, mapping = aes(y = ZAWMRP, color = "ZAWMRP"), size = 3)   # Change the color and size accordingly
  #       # Add other geom_point layers for other variables if needed
  #   } else {
  #     NULL
  #   } +
  #   #ggtitle("LICZBA ANOMALII W PODZIALE NA USŁUGI") +
